...
function createServer() {
    ...
    // TOOL: list_areas
    // ─────────────────────────────────────────────
    server.tool("list_areas", "Saare available categories (Areas) ki list dikhao", {}, async () => {
        const areas = await prisma.area.findMany({
            where: { is_active: true },
            orderBy: { name: "asc" },
        });
        if (areas.length === 0)
            return { content: [{ type: "text", text: "Koi category (Area) nahi mili." }] };
        const text = areas.map((a) => `🏷️ ${a.name} [${a.id}]`).join("\n");
    ...
    server.tool("update_task", "Existing task ko update karo — koi bhi field badal sakte ho", {
        ...
        if (args.recurrence)
            updateData.recurrence = args.recurrence === "none" ? null : args.recurrence;
        if (args.category && args.category !== "all") {
            const area = await prisma.area.findFirst({
                where: { name: { equals: args.category, mode: "insensitive" } },
            });
