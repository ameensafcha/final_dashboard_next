      ...
      <DialogContent className="sm:max-w-[420px] p-10 rounded-[var(--radius-xl)] border-none shadow-[var(--shadow-xl)] bg-[var(--glass-bg)] backdrop-blur-3xl overflow-hidden">
        <DialogHeader className="flex flex-col items-center text-center gap-6">
          ...
          <div className="space-y-3">
            ...
            <DialogDescription className="text-sm text-[var(--muted)] font-bold leading-relaxed">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-4 mt-10">
          <button
            onClick={onClose}
            disabled={isLoading}
            className="flex-1 rounded-full font-black text-[11px] uppercase tracking-widest text-[var(--muted)] hover:text-[var(--foreground)] transition-all h-12"
          >
            Abort
          </button>
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className="flex-1 rounded-full font-black text-[11px] uppercase tracking-[0.2em] bg-[var(--error)] hover:bg-[var(--error)]/90 text-white h-12 shadow-[var(--shadow-xl)] hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3"
          >
            {isLoading ? (
              <>
